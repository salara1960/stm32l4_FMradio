...
inline static void ST7565_w_dat(uint8_t Dat)
{
	//-- если захотим переделать под HAL ------------------	
		...
		if (with_dma) {
			lcdRdy = 0;
			if (HAL_SPI_Transmit_DMA(&ST7565_SPI_HAL, &Dat, 1) != HAL_OK) devError |= devLCD;
			while (!lcdRdy) {};
		} else {
			if (HAL_SPI_Transmit(&ST7565_SPI_HAL, &Dat, 1, HAL_MAX_DELAY) != HAL_OK) devError |= devLCD;
			//while(HAL_SPI_GetState(&ST7565_SPI_HAL) != HAL_SPI_STATE_READY) {};
		}
	#endif
	//-----------------------------------------------------
...
inline static void ST7565_w_cmd(uint8_t Command)
{
	//-- если захотим переделать под HAL ------------------	
		...
		// pin DC LOW
		 ...
		 if (with_dma) {
			 lcdRdy = 0;
			 if (HAL_SPI_Transmit_DMA(&ST7565_SPI_HAL, &Command, 1) != HAL_OK) devError |= devLCD;
			 while (!lcdRdy) {};
		 } else {
			 if (HAL_SPI_Transmit(&ST7565_SPI_HAL, &Command, 1, HAL_MAX_DELAY) != HAL_OK) devError |= devLCD;
		 	 //while (HAL_SPI_GetState(&ST7565_SPI_HAL) != HAL_SPI_STATE_READY) {};
		 }
...
void ST7565_Init()
{
	...
	ST7565_w_cmd(CMD_SET_RESISTOR_RATIO | 7);    //Regulator Resistor Selection
	//HAL_Delay(1);//1
	// управление питанием, все внутренние блоки включены	(от 0..7)
	ST7565_w_cmd(CMD_SET_POWER_CONTROL | 7);
	//HAL_Delay(1);//1
	...
	uint8_t dta[] = {
		...
		CMD_SET_ADC_NORMAL,
		CMD_SET_COM_REVERSE,
		CMD_SET_RESISTOR_RATIO | 7,
		CMD_SET_POWER_CONTROL | 7,
...
{
	memset(ST7565_buffer, 0, ((SCREEN_WIDTH * SCREEN_HEIGHT) >> 3));	// отчистка памяти заполняем массив значением 0x00
}
//--------------------------------------------------------------------------------
...
void ST7565_InvertDisplay(uint8_t i)
{
	...
	// Инвертирование экрана CMD_SET_DISP_NORMAL - OFF    CMD_SET_DISP_REVERSE - ON
	if (i) {
		byte = CMD_SET_DISP_REVERSE;
	} else {
		byte = CMD_SET_DISP_NORMAL;
	}
	ST7565_w_cmds(&byte, 1);
...
uint16_t i = 0;
	...
	while (*str != '\0') {
		if ((uint8_t)*str >= 0xC0) str++;// код 0xC0 соответствует символу кириллица 'A' по ASCII Win-1251
		i++;
		str++;
	}
